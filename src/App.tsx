...
function App() {
  ...
  // Facility options based on refrigerant
  ...
  const handleDiagnosis = async () => {
    ...
    const result = diagnoseSystem(
      refrigerant, lowP, highP, fieldStd, targetHighP,
      superheat, subcooling, compRatio, symptoms
    )
    ...
    try {
      const response = await getAIMentorResponse({
        ...
        lowPressure: lowP,
        highPressure: highP,
        targetHighPressure: targetHighP,
        fieldStandard: fieldStd,
        evapTemp,
        condTemp,
        superheat,
  ...
  const handleDownloadReport = () => {
    ...
    const reportText = generateReportText(
      diagnosisResult, refrigerant, facilityType, lowP, highP,
      targetHighP, fieldStd, superheat, subcooling
    )
      ...
      <div className="flex gap-4 p-4">
        ...
        <aside className="w-72 gradient-sidebar rounded-2xl p-4 text-white shrink-0">
          <div className="text-center mb-4">
            ...
            <p className="text-gray-400 text-sm">v2.0 React</p>
          </div>
            ...
            {facilityOptions.map(opt => (
              <label key={opt} className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="radio"
                  name="facility"
                  checked={facilityType === opt}
                  onChange={() => setFacilityType(opt)}
          ...
          <div className="bg-teal-900/30 p-3 rounded-lg mb-4">
            ...
            <p className="text-xl font-bold">
              {fieldStd.low_p_range[0]} ~ {fieldStd.low_p_range[1]} kg/cm²G
            </p>
          </div>
          ...
          <label className="flex items-center gap-2 text-purple-400 font-bold text-sm mb-2 cursor-pointer">
            <input
              type="checkbox"
              checked={usePipeTemp}
              onChange={(e) => setUsePipeTemp(e.target.checked)}
        ...
        <main className="flex-1">
          ...
          <div className="flex gap-2 bg-white p-2 rounded-xl shadow mb-4 flex-wrap">
            {tabs.map(tab => (
              <button
                ...
                className={`tab-btn flex items-center gap-1 ${activeTab === tab.id ? 'active' : ''}`}
              >
                <tab.icon size={16} />
                {tab.label}
              </button>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-600 mb-1">
                      🔵 저압 ({fieldStd.low_p_range[0]}~{fieldStd.low_p_range[1]} kg)
                    </label>
                    <input
                      type="number"
                      step="0.1"
                      value={lowP}
                      onChange={(e) => setLowP(Number(e.target.value))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      step="0.5"
                      value={highP}
                      onChange={(e) => setHighP(Number(e.target.value))}
                      className="input-field"
                    />
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-4">
                      ...
                      {usePipeTemp && (
                        <div className="metric-card">
                          ...
                          <p className="text-xl font-bold">
                            {calculateSuperheat(refrigerant, lowP, suctionTemp).superheat}°C
                          </p>
                        </div>
                      )}
                      <div className="metric-card">
                  ...
                  <div className="card">
                    ...
                    <div className="space-y-2 mb-4">
                      {diagnosisResult.issues.map((issue, i) => (
                        <p key={i} className="text-lg">{issue}</p>
                      ))}
                    </div>
                        ...
                        {aiResponse.includes('@@@') ? (
                          <>
                            ...
                            <details className="bg-blue-50 rounded-lg">
                              <summary className="p-4 cursor-pointer font-bold text-blue-800">
                                🎓 원리 설명 (클릭하여 펼치기)
                              </summary>
              ...
              <div className="space-y-4">
                {Object.entries(FAULT_PATTERNS).map(([key, pattern]) => (
                  <details key={key} className="bg-gray-50 rounded-lg">
                    <summary className="p-4 cursor-pointer font-bold hover:bg-gray-100 rounded-lg">
                      {key}
                    </summary>
                    <div className="p-4 pt-0 space-y-3">
                      <div>
                        <p className="font-semibold text-gray-700">가능한 원인:</p>
                        <p className="text-gray-600">{pattern.원인.join(', ')}</p>
                      </div>
                      <div>
                        <p className="font-semibold text-gray-700">동반 증상:</p>
                        <p className="text-gray-600">{pattern.증상.join(', ')}</p>
                      </div>
                      <div>
                        <p className="font-semibold text-gray-700">조치사항:</p>
                        <ul className="text-gray-600 list-none">
                          {pattern.조치.map((a, i) => <li key={i}>{a}</li>)}
                        </ul>
                      </div>
                      <div className="p-3 bg-yellow-100 rounded-lg">
                        <p className="font-semibold text-yellow-800">⚠️ {pattern.주의}</p>
                      </div>
                    </div>
                  </details>
                ))}
              ...
              <div className="space-y-4">
                {Object.entries(EMERGENCY_MANUAL).map(([key, item]) => (
                  <div key={key} className="border-l-4 border-red-500 pl-4 py-2">
                    ...
                    <div className="mb-2">
                      <p className="font-semibold">즉시 조치:</p>
                      <ul className="text-gray-700 list-none">
                        {item.즉시조치.map((a, i) => <li key={i}>{a}</li>)}
                      </ul>
                    </div>
          ...
          {activeTab === 'charging' && (
            <div className="card">
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="font-bold text-indigo-600 mb-2">충전 전 준비</h4>
                  <ul className="space-y-1">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {Object.entries(CHARGING_GUIDE["충전 방법"]).map(([ref, methods]) => (
                      <div key={ref} className="bg-gray-50 p-3 rounded-lg">
                        <p className="font-bold mb-2">{ref}</p>
                ...
                <div>
                  <h4 className="font-bold text-indigo-600 mb-2">충전 중 확인</h4>
                  <ul className="space-y-1">
                    {CHARGING_GUIDE["충전 중 확인"].map((item, i) => (
                      <li key={i} className="text-gray-700">{item}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  <h4 className="font-bold text-green-700 mb-2">충전 완료 기준</h4>
                  <ul className="space-y-1">
                ...
                <div className="bg-red-50 p-4 rounded-lg">
                  <h4 className="font-bold text-red-700 mb-2">주의사항</h4>
                  <ul className="space-y-1">
                    {CHARGING_GUIDE["주의사항"].map((item, i) => (
